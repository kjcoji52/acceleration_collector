...
float mma8451_convert_gravity(uint16_t raw_accel)
{
  float gravity;
  ...
  {
    raw_accel &= 0x1FFF;
    gravity = (-8192 + (float)raw_accel) / count;
  }
  else
  {
    gravity = (float)raw_accel / count;
  }
  return gravity;
}
...
void mma451_read_gravity(MMA8451Gravity *gravity)
{
  // Start reading gravity registers for 2 bytes
